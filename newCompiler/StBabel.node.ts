...
export class StBabel {
    protected mode: "dev" | "pro" = "dev";
    public options: babel.TransformOptions = {
    ...
    public babelFile(filename) {
        let parser = this;
        stEcho.log(`准备编绎 \n ${filename} \n on ${parser.options.sourceMaps}`, "purple");
        return new StPromise((resolve, reject) => {
            babel.transformFile(filename, parser.options, (err, result) => {
                if (err) {                    
    ...
    public writeOn(file, code) {
        ...
        fileSys.writeFile(file, code, { encoding: "utf8" }, err => {
            if (err) {
                ...
                stEcho.info(`成功写入${file}`);
            }
        });
    }
}
export let stBabel = new StBabel();
